package com.z4greed.inventory.kafka.config;
...
public class KafkaConsumerRetryConfig {
  ...
  // Ejemplo: inventory-commands-topic se convierte en inventory-commands-topic-dlt y conserva la partición.
  ...
  // Declara el DLT asociado al topic de comandos que consume Inventory Service.
  // Los reintentos no crean otro topic: DefaultErrorHandler vuelve a procesar el mismo mensaje
  // desde inventory-commands-topic. Solo después de agotar los intentos lo publica en este DLT.
  // La relación es por nombre: inventory-commands-topic + "-dlt".
  @Bean
  public NewTopics inventoryDeadLetterTopics() {
    NewTopic newTopic = TopicBuilder.name("inventory-commands-topic-dlt")
